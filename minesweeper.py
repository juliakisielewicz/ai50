...
class MinesweeperAI():
        ...
        if len(neighbors) > 0:
            ...
            for sentence in self.knowledge:
                if new.cells.issubset(sentence.cells):
                    conclusion = Sentence(sentence.cells.difference(new.cells), sentence.count - new.count)
                    if conclusion not in self.knowledge:
                        self.knowledge.append(conclusion)
                elif sentence.cells.issubset(new.cells):
                    conclusion = Sentence(new.cells.difference(sentence.cells), new.count - sentence.count)
                    if conclusion not in self.knowledge:
                        self.knowledge.append(conclusion)
    ...
    def make_safe_move(self):
        ...
        """
        possibles = self.safes.difference(self.moves_made)
        if bool(possibles):
    ...
    def make_random_move(self):
        ...
        """
        count = self.height * self.width - len(self.moves_made.union(self.mines))
        while count > 0:
